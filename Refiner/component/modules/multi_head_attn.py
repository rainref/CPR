...
from component.utils.misc import generate_relative_positions_matrix, \
    relative_matmul

class SpatialDepthWiseConvolution(nn.Module):
    def __init__(self, d_k: int, kernel_size: int = 3,head_count=8):
        super().__init__()
    ...
    def forward(self, key, value, query, mask=None, layer_cache=None,
        ...
        if layer_cache is not None:
            if attn_type == "self":
                ...
                query = shape(self.query(query), self.d_k)
                if layer_cache["gnn_keys"] is None:
                    key = shape(self.key(key), self.d_k)
                    value = shape(self.value(value), self.d_v)
                else:
                    key, value = layer_cache["gnn_keys"], \
                                 layer_cache["gnn_values"]
                layer_cache["gnn_keys"] = key
                layer_cache["gnn_values"] = value
...
class MultiDConvHeadAttention(MultiHeadedAttention):
    def __init__(self, head_count, model_dim, d_k, d_v, dropout=0.1,
                 max_relative_positions=0, use_neg_dist=True, coverage=False):
        super().__init__(head_count, model_dim, d_k, d_v, dropout=dropout,
                 max_relative_positions=max_relative_positions, use_neg_dist=use_neg_dist, coverage=coverage)
