...
from transformers import OPTModel,AutoModelForSeq2SeqLM,AutoModelForMaskedLM
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        if args.model_type == "OPT":
            self.transformer = OPTModel.from_pretrained("../../models/opt").decoder
            self.out_lin = nn.Linear(768, args.emsize)
            self.in_lin = nn.Linear(args.emsize, 768)
        elif args.model_type == "CodeT5":
            self.transformer = AutoModelForSeq2SeqLM.from_pretrained("../../models/CodeT5").encoder
            self.out_lin = nn.Linear(512, args.emsize)
            self.in_lin = nn.Linear(args.emsize, 512)
    ...
    def forward(self,
        ...
        if self.args.model_type=="transformer":
            ...
            return memory_bank,memory_bank

        mask = sequence_mask(input_len, input.shape[1])

        input = self.in_lin(input)

        out = self.transformer(inputs_embeds=input, attention_mask=mask)  # B x seq_len x h

        out = self.out_lin(out["last_hidden_state"])
